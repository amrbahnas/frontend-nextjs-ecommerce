...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    // Product routes
    const productRoutes = (Array.isArray(products) ? products : []).map(
      (product: any) => ({
    ...
    // Category routes
    const categoryRoutes = (Array.isArray(categories) ? categories : []).map(
      (category: any) => ({
        url: `${baseUrl}/list?category=${category.id}`,
        lastModified: new Date().toISOString(),
